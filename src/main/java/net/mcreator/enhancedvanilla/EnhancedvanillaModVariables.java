package net.mcreator.enhancedvanilla;
...
public class EnhancedvanillaModVariables {
	public static class WorldVariables extends WorldSavedData {
		...
		public void syncData(World world) {
			this.markDirty();
			if (world.isRemote) {
				EnhancedvanillaMod.PACKET_HANDLER.sendToServer(new WorldSavedDataSyncMessage(1, this));
			} else {
				EnhancedvanillaMod.PACKET_HANDLER.send(PacketDistributor.DIMENSION.with(world.dimension::getType),
	...
	public static class MapVariables extends WorldSavedData {
		public static final String DATA_NAME = "enhancedvanilla_mapvars";
		public MapVariables() {
			super(DATA_NAME);
		}
		...
		public void syncData(World world) {
			this.markDirty();
			if (world.isRemote) {
				EnhancedvanillaMod.PACKET_HANDLER.sendToServer(new WorldSavedDataSyncMessage(0, this));
			} else {
				EnhancedvanillaMod.PACKET_HANDLER.send(PacketDistributor.ALL.noArg(), new WorldSavedDataSyncMessage(0, this));
			}
		}
	...
	public static class WorldSavedDataSyncMessage {
		...
		private static void syncData(WorldSavedDataSyncMessage message, LogicalSide side, World world) {
			if (side.isServer()) {
				...
				if (message.type == 0) {
					EnhancedvanillaMod.PACKET_HANDLER.send(PacketDistributor.ALL.noArg(), message);
					world.getServer().getWorld(DimensionType.OVERWORLD).getSavedData().set(message.data);
				} else {
					EnhancedvanillaMod.PACKET_HANDLER.send(PacketDistributor.DIMENSION.with(world.dimension::getType), message);
